...
// v7.5.2-4 single precise page: defaults for spouse/deps; newused restored; manual inputs disabled until checked
...
function compute(){
  ...
  const need=[]; [['salary','年収'],['age','年齢'],['targetY','対象年'],['spIncome','配偶者の合計所得金額'],['depGen','扶養（一般）人数'],['depSpec','扶養（特定）人数'],['depElderSame','老人扶養（同居）人数'],['depElderOther','老人扶養（その他）人数'],['startYM','借入開始年月'],['principal','借入総額'],['years','返済年数'],['rate','年利']].forEach(([id,label])=>{const el=document.getElementById(id);if(isEmpty(el.value))need.push(label)});
  if(!pref)need.push('都道府県'); if(!ok_ym(ym))need.push('借入開始年月の形式');
  if(need.length){err('入力不備：'+need.join('、')+' を入力・確認してください。');return;}
  ...
  const cap=30000000, idx=ty-sy; if(idx<0||idx>=13){err('対象年は控除期間外の可能性があります。');return;}
...
function resetAll(){document.querySelectorAll('input').forEach(el=>{if(el.type==='checkbox'){el.checked=false}else if(['spIncome','depGen','depSpec','depElderSame','depElderOther'].includes(el.id)){el.value='0'}else el.value=''});document.querySelectorAll('select').forEach(el=>{if(el.id==='pref')el.value='東京都';else el.selectedIndex=0});document.getElementById('R-man').disabled=true;document.getElementById('R-man').style.display='none';document.getElementById('yb-man').disabled=true;document.getElementById('yearend-man').style.display='none';document.getElementById('resultBox').classList.remove('show');clearErr();showHint()}
